...
for m in models:
	...
	for i in range(2):
		...
		if(i==0):
			ax.plot(range(len(avg_r)),avg_r, markersize=3, marker="o",color=colors[i], label=labels[i])
			ax.fill_between(range(len(avg_r)), lbs_r, ubs_r, alpha=0.2,color=colors[i])
			plt.ylim([0, 2500])
			...
			ax.plot(range(len(avg_s)),avg_s, markersize=3, marker="o",color=colors[i], label=labels[i])
			ax.fill_between(range(len(avg_s)), lbs_s, ubs_s, alpha=0.2,color=colors[i])
